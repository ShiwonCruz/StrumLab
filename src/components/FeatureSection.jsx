import { features } from "../constants";
import { motion } from "framer-motion";

const FeatureSection = () => {
  return (
    <div className=" relative mt-52 border-b border-neutral-800 min-h-[800px]" id="features">
      <div className="text-center">
        <span className=" bg-neutral-900 text-sky-500 rounded-full h-6 text-sm font-medium px-2 py-1 uppercase">
          Highlights
        </span>
        <h2 className="text-3xl sm:text-5xl lg:text-6xl mt-10 lg:mt-20 tracking-wide">
          Here's what makes us{" "}
          <span className="bg-gradient-to-r  from-emerald-500 to-blue-500 text-transparent bg-clip-text">
            Special
          </span>
        </h2>
      </div>
      <div className="flex flex-wrap mt-10 lg:mt-20">
        {features.map((feature, index) => (
          <motion.div key={index} className="w-full sm:w-1/2 lg:w-1/3" whileHover={{scale:1.1}}>
            <div className="flex">
              <div className="flex mx-6 h-10 w-10 p-2 bg-neutral-900 text-green-400 justify-center items-center rounded-full">
                {feature.icon}
              </div>
              <div>
                <h5 className="mt-1 mb-6 text-xl">{feature.text}</h5>
                <p className="text-md p-2 mb-20 text-neutral-500">
                  {feature.description}
                </p>
              </div>
            </div>
          </motion.div>
        ))}
      </div>
    </div>
  );
};

export default FeatureSection;
